...
let selectedMods = [];   // ← now an array for multiple select
...
onAuthStateChanged(auth, async (user) => {
    ...
    if (autoFile) {
        ...
        // ← Push into array instead of single assignment
        selectedMods = [{
        ...
        files.forEach(file => {
            ...
            card.addEventListener('click', () => {
                ...
                if (card.classList.contains('selected')) {
                    ...
                    selectedMods = selectedMods.filter(m => m.fileName !== file.name);
                } else {
                    card.classList.add('selected');
                    selectedMods.push(modObj);
                }
                document.getElementById('btn-to-setup').disabled = selectedMods.length === 0;
            });
            ...
            card.querySelector('.mod-delete-btn').addEventListener('click', async (e) => {
                ...
                // ← Remove from array if it was selected
                selectedMods = selectedMods.filter(m => m.fileName !== file.name);
                document.getElementById('btn-to-setup').disabled = selectedMods.length === 0;
            });
...
document.getElementById('btn-to-setup').addEventListener('click', () => {
    const names = selectedMods.map(m => m.displayName || getDisplayName(m.fileName));
    document.getElementById('selected-mod-name').textContent = names.join(', ');
    // Use layer icon when multiple, otherwise use file icon
    if (selectedMods.length > 1) {
        document.querySelector('.selected-mod-bar i').className = 'fas fa-layer-group';
    } else {
...
//  GENERATE — fetches from each selected module and combines
document.getElementById('btn-generate').addEventListener('click', async () => {
    ...
    try {
        const allQuestions = [];
        const perModule = Math.ceil(totalQ / selectedMods.length);

        for (const mod of selectedMods) {
            const { data: fileBlob, error: dlErr } = await supabase.storage
                .from('modules')
                .download(mod.supabasePath);

            if (dlErr) throw new Error(`Could not download ${mod.displayName}: ${dlErr.message}`);
            setGenStep(2);

            const formData = new FormData();
            formData.append('file', new File([fileBlob], mod.displayName, { type: 'application/pdf' }));
            formData.append('quiz_type', typeMap[currentType]);
            formData.append('question_count', perModule.toString());
            setGenStep(3);

            const res = await fetch(API_URL, { method: 'POST', body: formData });
            if (!res.ok) throw new Error(`Server error for ${mod.displayName}: ${res.status}`);

            const data = await res.json();
            if (data.quiz && data.quiz.length) allQuestions.push(...data.quiz.slice(0, perModule));
        }

        if (!allQuestions.length) throw new Error('No questions returned. Try a different file.');
        setGenStep(4);

        questions = allQuestions.slice(0, totalQ).map(q => ({
            q:       q.question,
...
function renderAnswerReview() {
    ...
    answerLog.forEach(a => {
        ...
        row.innerHTML = `
            ...
            <div class="res-row-score ${a.is_correct ? 'correct' : 'wrong'}">
                ${a.is_correct ? '✓' : '✗'}
            </div>`;
...
async function showResults() {
    ...
    try {
        ...
        // ← Join all module names for display
        const displayName = selectedMods.map(m => m.displayName || getDisplayName(m.fileName)).join(', ');
